...
function writeRepoConfig(sanctions: unknown[], files: unknown[] = []): void {
  writeFileSync(
    path.join(root, 'guardrails.config.json'),
    JSON.stringify({
      sanctionedSuppressions: sanctions,
      sanctionedFiles: files,
    }),
  );
...
describe('runCommand — sanctions-check (CI approval-visibility gate)', () => {
  ...
  it('passes and prominently prints each newly-granted exemption', async () => {
    ...
    // The converse of the whole-file case: a branch adding only a keyed grant
    // must not be told it granted a whole-file exemption.
    expect(printed).not.toContain('WHOLE-FILE');
...
describe('runCommand — sanctions-check with whole-file grants', () => {
  const GENERATED = {
  ...
  it('prints a new whole-file grant under its own heading', async () => {
    // A path grant is the broader of the two forms -- every occurrence of a
    // kind in a file, forever, with no count bounding it. A reviewer skimming
    // the check output must not have to work out which kind they are
    // approving, so it is labelled rather than mixed in with keyed grants.
    writeRepoConfig([], [GENERATED]);
    const base = JSON.stringify({ sanctionedSuppressions: [] });
    ...
    expect(printed).toContain('src/routeTree.gen.ts');
    expect(printed).toContain('cast-any');
    expect(printed).toContain('never hand-edited');
    // And ONLY that heading: a branch adding no keyed grant must not be told
    // it granted one.
    expect(printed).not.toContain('new diff-auditor exemption(s) granted on');
  });

  it('says nothing new when the grant already exists on the base', async () => {
    writeRepoConfig([], [GENERATED]);
    const base = JSON.stringify({ sanctionedFiles: [GENERATED] });
  ...
  it('never reports count drift for a whole-file grant', async () => {
    // The defect this grant form exists to remove (#39): a keyed grant's count
    // must match the source exactly, so a generated file forces the adopter to
    // bump a number on every regeneration. Nothing here materialises the file,
    // so a path grant subjected to the drift check would report "declared 1,
    // found 0" and fail the build.
    writeRepoConfig([], [GENERATED]);
    const base = JSON.stringify({ sanctionedFiles: [GENERATED] });
  ...
  it('fails on a malformed whole-file grant rather than ignoring it', async () => {
    // An unknown kind grants nothing while looking exactly like a grant that
    // works. Reported, not dropped in silence.
    writeRepoConfig([], [{ ...GENERATED, kind: 'not-a-kind' }]);
  ...
  it('reports both grant kinds together when a branch adds each', async () => {
    writeRepoConfig([REQUESTED], [GENERATED]);
    const base = JSON.stringify({ sanctionedSuppressions: [] });
