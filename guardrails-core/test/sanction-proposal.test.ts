...
describe('proposeSanctions', () => {
  ...
  it('names sanctionedFiles for a generated file, and says why', () => {
    ...
    expect(proposal?.mechanism).toBe('sanctionedFiles');
    expect(proposal?.entry).toEqual({
      path: 'src/routeTree.gen.ts',
      kind: 'cast-any',
      reason: '',
    });
    expect(proposal?.mechanismNote).toContain('regeneration');
  });
  ...
  ])('recognises %s as generated from its path alone', (file) => {
    // No header in the source: the path is the whole signal here.
    expect(
      proposeSanctions(auditSource(file, CAST_LINE), context())[0]?.mechanism,
    ).toBe('sanctionedFiles');
  });

  it('does not read a hand-written file as generated', () => {
    // The negative half of the pair: `sanctionedFiles` is the broader grant,
    // so over-matching here hands out the un-counted exemption for code a
    // person wrote and will keep editing.
    expect(
      proposeSanctions(
        auditSource('src/regenerate.ts', CAST_LINE),
        context(),
      )[0]?.mechanism,
    ).toBe('sanctionedSuppressions');
  });
  ...
  ])('recognises a generated file by the header %s', (header) => {
  ...
  it('keeps the counted mechanism for hand-written code, however many occurrences', () => {
    ...
    expect(proposal?.mechanism).toBe('sanctionedSuppressions');
