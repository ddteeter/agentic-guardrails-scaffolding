...
function parseSkill(name) {
  const source = readFileSync(
    path.join(skillsFrom, name, 'SKILL.md'),
    'utf8',
  );
...
if (skillList.length > 0) {
  ...
  for (const skill of skillList) {
    writeFileSync(
      path.join(skillDocs, `${skill.name}.md`),
      [
...
const index = [
  ...
  '',
  ...skillList.map(
    (skill) => `- [\`${skill.name}\`](../docs/guardrails/${skill.name}.md) — ${skill.description}`,
  ),
  '',
  END,
].join('\n');
...
console.log(
  `synced ${skillList.length} skill doc(s): guardrails-plugin/skills → docs/guardrails + .github/copilot-instructions.md`,
);
